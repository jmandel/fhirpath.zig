...
DEFAULT_FILE = ROOT / "wiggum" / "bug_backlog.yaml"
...
def load_bugs(path: Path) -> dict:
    ...
    return {"version": version, "bugs": bugs}


def save_bugs(path: Path, data: dict) -> None:
    lines = [
        "# Bug Backlog (YAML)",
        "#",
    ...
    for bug in data.get("bugs", []):
        lines.append(f"  - slug: {_yaml_escape(str(bug.get('slug', '')))}")
        for key in ("status", "severity", "title", "description", "created", "updated"):
            if key in bug:
                lines.append(f"    {key}: {_yaml_escape(str(bug.get(key, '')))}")
        tags = bug.get("tags")
        if tags is not None:
...
def list_bugs(data: dict, show_all: bool):
    for bug in data.get("bugs", []):
...
def show_bug(data: dict, slug: str):
    for bug in data.get("bugs", []):
...
def find_bug(data: dict, slug: str):
    for bug in data.get("bugs", []):
        if bug.get("slug") == slug:
            return bug
    return None
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage bug_backlog.yaml")
    parser.add_argument("--file", default=str(DEFAULT_FILE), help="Path to bug_backlog.yaml")
    ...
    p_show = sub.add_parser("show", help="Show bug by slug")
    p_show.add_argument("slug")

    p_add = sub.add_parser("add", help="Add a bug")
    ...
    p_update = sub.add_parser("update", help="Update a bug by slug")
    ...
    p_resolve = sub.add_parser("resolve", help="Mark bug resolved")
    ...
    if args.cmd == "list":
        list_bugs(data, args.all)
        return 0

    if args.cmd == "show":
        ok = show_bug(data, args.slug)
    ...
    if args.cmd == "add":
        ...
        if find_bug(data, slug):
            print(f"Bug already exists: {slug}")
            return 1
        status = args.status
    ...
    if args.cmd in ("update", "resolve"):
        slug = args.slug
        bug = find_bug(data, slug)
        if not bug:
            print(f"Bug not found: {slug}")
            return 1
        if args.cmd == "resolve":
            bug["status"] = "resolved"
        else:
            ...
            if args.tags is not None:
                bug["tags"] = [t.strip() for t in args.tags.split(",") if t.strip()]
        bug["updated"] = today
        save_bugs(path, data)
        return 0
